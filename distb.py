...
parser.add_argument(
    "--data_path",
    type=str,
    default="/rscratch/tpang/kinshuk/RpMKin/distilbert/dataset/english/data.csv",
...
def val_loss(model, data_loader, device):
    ...
    return total_loss / len(data_loader), val_acc / input_len
...
def tr_loss(model, train_dataloader, val_dataloader, optimizer, device):
    ...
    for epoch in range(args.epochs):
        ...
        if epoch == 0:
            vl, va = val_loss(model, val_dataloader, device)
        ...
        tr_loss = total_loss / len(train_dataloader)
        print(f"Epoch {epoch+1}, Train Loss: {tr_loss}")
        v_loss, v_acc = val_loss(model, val_dataloader, device)
        print(f"Epoch {epoch+1}, Val Loss: {v_loss}, Val Acc: {v_acc}")
        tr.append(tr_loss)
        val.append(v_loss)
        acc.append(v_acc)
    return tr, val, acc
...
def main():
    ...
    # cll, vll = get_train_val(
    #     args, df_val, batch_size=32, val_ratio=0.2, fraction=1
    # )
    ...
    print("Training...")
    train_loss, val_loss, val_acc = tr_loss(model, train_dataloader, val_dataloader, optimizer, device)
